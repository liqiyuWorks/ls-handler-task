...
class P5Parser:
    ...
    def _extract_p5_profit_loss_ratio(self, text: str) -> Dict[str, Any]:
        """提取P5盈亏比数据"""
        p5_data = {
            "current_price": None,
        ...
        if current_price_match:
            p5_data["current_price"] = int(current_price_match.group(1))
        
        # 提取评估价格
        evaluated_price_match = re.search(r'评估价格/元每吨[：:]\s*(\d+)', text)
        if evaluated_price_match:
            p5_data["evaluated_price"] = int(evaluated_price_match.group(1))
        
        # 提取价差比
        price_diff_match = re.search(r'价差比[：:]\s*(-?\d+%)', text)
        if price_diff_match:
            p5_data["price_difference_ratio"] = price_diff_match.group(1)
        
        # 提取42天后盈利比例和收益均值
        profit_ratio_match = re.search(r'42天后盈利比例[：:]\s*(\d+)%', text)
        if profit_ratio_match:
            p5_data["profit_ratio_42d"] = int(profit_ratio_match.group(1))
        
        profit_avg_match = re.search(r'收益均值[：:]\s*(\d+)%', text)
        if profit_avg_match:
            p5_data["profit_average_42d"] = int(profit_avg_match.group(1))
        
        # 提取42天后亏损比例和亏损均值
        loss_ratio_match = re.search(r'42天后亏损比例[：:]\s*(\d+)%', text)
        if loss_ratio_match:
            p5_data["loss_ratio_42d"] = int(loss_ratio_match.group(1))
        
        loss_avg_match = re.search(r'亏损均值/元每吨[：:]\s*(\d+)%', text)
        if loss_avg_match:
            p5_data["loss_average_42d"] = int(loss_avg_match.group(1))
        ...
        for pattern, key in max_profit_timing_patterns:
            match = re.search(pattern, text)
            if match:
                p5_data["max_profit_timing_distribution"][key] = int(match.group(1))
        
        # 提取最大风险均值
        max_risk_avg_match = re.search(r'最大风险均值/元每吨[：:]\s*(-?\d+%)', text)
        if max_risk_avg_match:
            p5_data["max_risk_average"] = max_risk_avg_match.group(1)
        
        # 提取最大风险极值
        max_risk_extreme_match = re.search(r'最大风险极值/元每吨[：:]\s*(-?\d+%)', text)
        if max_risk_extreme_match:
            p5_data["max_risk_extreme"] = max_risk_extreme_match.group(1)
        ...
        for pattern, key in max_risk_timing_patterns:
            match = re.search(pattern, text)
            if match:
                p5_data["max_risk_timing_distribution"][key] = int(match.group(1))
        
        return p5_data
    
    def _extract_p5tc_model_evaluation(self, text: str) -> Dict[str, Any]:
        """提取P5TC六周后预测模型评价数据"""
        evaluation = {
            "date": None,
            "current_price": None,
