...
def get_task_dic():
    ...
    task_dict = {
        ...
        "gfs_realtime_sync_mgo": (lambda: CustomScheduler(GfsRealtimeSyncMgo).run(),'同步GFS实时监测的数据到mgo'),
        "gfs_forecast_sync_mgo": (lambda: CustomScheduler(GfsForecastSyncMgo).run(),'同步GFS预报的数据到mgo'),
