...
def get_task_dic():
    ...
    task_dict = {
        ...
        "gfs_realtime_sync_mgo": (lambda: CustomScheduler(TyphoonSyncMgo).run(),'同步实时监测台风的数据到mongo'),
        "gfs_forecast_sync_mgo": (lambda: CustomScheduler(GfsSyncMgo).run(),'同步GFS预报的数据到mongo'),
        "spider_currMerger2json": (lambda: CustomScheduler(SpiderCurrmergerJson).run(), "下载台风数据，生成json"),
        "wztfw_sync_mgo": (lambda: CustomScheduler(WZCurrSyncMgo).run(), "同步温州台风实时和预报数据！"),
        "history_wztfw_sync_mgo": (lambda: WZCurrSyncMgo().history(), "一次性同步温州台风实时和预报数据(需传HISTORY_YEAR)"),
        "history_gfs_forecast_sync_mgo": (lambda: GfsSyncMgo().history(),'一次性导入GFS的历史数据(需传HISTORY_YEAR)'),
    }
    return task_dict
