...
def read_era5_nc(input_path):
    ...
    with Dataset(input_path) as nc_obj:
        ...
        print(len(latitude_li))
        try:
            u10_li = np.array(nc_obj["WIND_U10"][:])
            v10_li = np.array(nc_obj["WIND_V10"][:])
            time = np.array(nc_obj["time"][0])
        except Exception as e:
            print(e)
        ...
        for lat_index in range(0, len(latitude_li)):
            for lon_index in range(0, len(longitude_li)):
                u10 = float(values_dict['u10_li'][0][lat_index][lon_index])
                v10 = float(values_dict['v10_li'][0][lat_index][lon_index])
                array_u.append(round(u10, 2))
                array_v.append(round(v10, 2))

        print(
            f">>> {dt} {len(latitude_li)}, {len(longitude_li)}, u={array_u[0]}, v={array_v[0]}"
        )
...
if __name__ == '__main__':
    ...
    input_path = "/hpcdata/data_service/history/ERA5/0p25/20220718/ERA5_2022071801_0p25.nc"
