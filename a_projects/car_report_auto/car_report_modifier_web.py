...
class CarReportModifier:
    ...
    async def modify_qr_codes(self, config):
        """修改页面中的二维码图片 - 专门针对'官方验证'、'专属保障'、'交易保障'和'报告验真'四个二维码"""
        try:
            logger.info("🔄 开始替换页面中的二维码（官方验证 + 专属保障 + 交易保障 + 报告验真）...")
            ...
            # 使用JavaScript直接查找并替换四个特定的二维码
            logger.info("🔍 使用JavaScript查找并替换'官方验证'、'专属保障'、'交易保障'和'报告验真'二维码...")
            
            result = await config.page.evaluate(f"""
                () => {{
                    try {{
                        const targetTexts = ['官方验证', '专属保障', '交易保障', '报告验真'];
                        let modifiedCount = 0;
                        let results = [];
