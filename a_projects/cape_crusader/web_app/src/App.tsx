...
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, AreaChart, Area, ComposedChart, Bar, ReferenceLine, BarChart, Cell } from 'recharts';
import { Upload, Activity, ArrowRight, TrendingUp, TrendingDown, Crosshair, DollarSign, Settings, Play, List, AlertCircle, BarChart3, Sparkles, Check, Eye, EyeOff, Layers, FileText, Clock } from 'lucide-react';
...
const parseCSV = (text) => {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        const dateObj = new Date(parseInt(ymd[0]), parseInt(ymd[1]) - 1, parseInt(ymd[2]), parseInt(parts[1].split(':')[0]));
        ...
        let expiryDate = null;
        let contractMonthKey = null; // e.g. "Oct 2024"
        ...
        rawData.push({
            ...
            timeToExpiry: (expiryDate as any) - (dateObj as any)
        });
    }
...
const processTimeWeightedData = (rawData, selectedProduct) => {
    ...
    productData.forEach(d => {
        // Keep full timestamp for 4H precision
    ...
    sortedDates.forEach(dateKey => {
        ...
        // For each timestamp, find contracts
        const contractsMap = {};
        records.forEach(r => contractsMap[r.expiry.getTime()] = r);
        const uniqueContracts = Object.values(contractsMap).sort((a: any, b: any) => a.timeToExpiry - b.timeToExpiry);

        if (uniqueContracts.length >= 2) {
            const m1 = uniqueContracts[0];
            const m2 = uniqueContracts[1];

            const monthStart = new Date((m1 as any).expiry);
            monthStart.setDate(1);

            const totalDuration = (m1 as any).expiry - (monthStart as any);
            const timeLeft = (m1 as any).expiry - (m1 as any).date;

            let w2 = 1 - (timeLeft / totalDuration);
            w2 = Math.max(0, Math.min(1, w2));
            const w1 = 1 - w2;

            const synthClose = (m1 as any).close * w1 + (m2 as any).close * w2;
            const synthHigh = (m1 as any).high * w1 + (m2 as any).high * w2;
            const synthLow = (m1 as any).low * w1 + (m2 as any).low * w2;

            dailyData.push({
                date: (m1 as any).date, // Date Object
                dateStr: dateKey,
                close: synthClose,
                high: synthHigh,
                low: synthLow,
                spread: (m1 as any).close - (m2 as any).close,
                weight_m2: w2,
                M1_Price: (m1 as any).close,
                M2_Price: (m2 as any).close,
...
const processSingleContractData = (rawData, selectedProduct, selectedContract) => {
    ...
    // Group by timestamp (no heavy synthesis needed)
    const grouped = {};
...
const resampleData = (data, timeframe) => {
    if (timeframe === '4H') return data; // Default
    ...
    keys.forEach(k => {
        ...
        // Average spread/weight for simplicity in chart
        const avgSpread = bars.reduce((sum, b) => sum + b.spread, 0) / bars.length;

        resampled.push({
            ...last,
            date: new Date(k), // Use group start date
            dateStr: k,
            high,
            low,
            open: first.close, // Approximation using prev close or first record
            spread: avgSpread
        });
    });
...
const calculateIndicators = (data, params) => {
    ...
    const enriched = data.map((d, i) => {
        ...
        const macdLine = emaFast - emaSlow; // DIF

        if (i === 0) emaSignal = macdLine;
        else emaSignal = (macdLine - emaSignal) * alphaSig + emaSignal; // DEA
        ...
        // Use temp variable to capture value for this bar before reset
        let currentBc = 0;
        let currentSc = 0;
        ...
        return {
            ...
            td: {
                ...
                bs: bs > 0 ? bs : null, // Setup Count
                ss: ss > 0 ? ss : null, // Setup Count
                bc: currentBc > 0 ? currentBc : null, // Countdown
                sc: currentSc > 0 ? currentSc : null  // Countdown
            }
        };
    });
...
const runBacktest = (data, params) => {
    ...
    const days = data.length > 0 ? ((new Date(data[data.length - 1].date) as any) - (new Date(data[0].date) as any)) / (1000 * 3600 * 24) : 0;
...
const SignalMarker = (props) => {
    ...
    // Setup: 1-9
    // Buy Setup (Green, Below)
    if (bs) {
        markers.push(
            <text key="bs" x={cx} y={cy + 15} textAnchor="middle" fill="#22c55e" fontSize={bs === 9 ? 12 : 9} fontWeight={bs === 9 ? "bold" : "normal"}>{bs}</text>
        );
    }
    // Sell Setup (Red, Above)
    if (ss) {
        markers.push(
            <text key="ss" x={cx} y={cy - 10} textAnchor="middle" fill="#ef4444" fontSize={ss === 9 ? 12 : 9} fontWeight={ss === 9 ? "bold" : "normal"}>{ss}</text>
        );
    }

    // Countdown: 1-13 (Usually placed further out)
    // Buy Countdown (Dark Green, Further Below)
    if (bc) {
        markers.push(
            <text key="bc" x={cx} y={cy + 28} textAnchor="middle" fill="#15803d" fontSize={bc === 13 ? 12 : 9} fontWeight={bc === 13 ? "bold" : "normal"}>{bc}</text>
        );
    }
    // Sell Countdown (Dark Red, Further Above)
    if (sc) {
        markers.push(
            <text key="sc" x={cx} y={cy - 22} textAnchor="middle" fill="#b91c1c" fontSize={sc === 13 ? 12 : 9} fontWeight={sc === 13 ? "bold" : "normal"}>{sc}</text>
        );
    }
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {trades.slice().reverse().map((t, i) => (
                            <tr key={i} className="hover:bg-slate-50">
                                <td className="px-6 py-3 font-medium text-slate-900 whitespace-nowrap">{t.date}</td>
...
export default function App() {
    ...
    const handleFileUpload = (event) => {
        ...
        reader.onload = (e) => {
            try {
                ...
                setRawData(res.data);
                setMetadata({ products: res.products, minDate: res.minDate, maxDate: res.maxDate, allContracts: [] });
                const defProd = res.products.find((p: any) => p.includes('Capesize') || p.includes('5TC')) || res.products[0];
                const defRange = { start: res.minDate, end: res.maxDate };
                setDraftConfig({ product: defProd as string, ...defRange, mode: "timeWeighted", contract: "" });
    ...
    // Format XAxis tick based on timeframe
    const formatXAxis = (tickItem) => {
        if (!tickItem) return '';
        // If dataStr exists (from resampler), try to parse it
        // Depending on timeframe, we want concise labels
        if (timeframe === '4H') return tickItem.substring(5, 16).replace('T', ' '); // MM-DD HH:mm
        return tickItem.substring(0, 10); // YYYY-MM-DD
    };
        ...
        <div className="min-h-screen bg-slate-50 font-sans text-slate-800">
            <header className="bg-white border-b border-slate-200 sticky top-0 z-20 shadow-sm px-6 py-3 flex justify-between items-center">
                <div className="flex items-center gap-2">
                    ...
                    <h1 className="text-xl font-bold text-slate-800">Cape Crusader <span className="text-slate-400 font-normal">v3.6</span></h1>
                </div>
                        ...
                        {strategyData ? (
                            <>
                                ...
                                <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 flex flex-col gap-4">
                                    ...
                                    <div style={{ height: layers.macd ? '400px' : '500px' }} className="w-full transition-all duration-300">
                                    ...
                                    {layers.macd && (
                                        <div className="h-40 w-full border-t border-slate-50 pt-2">
                                            <ResponsiveContainer width="100%" height="100%">
