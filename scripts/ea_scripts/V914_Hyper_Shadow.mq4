...
extern int    Max_Dev_From_MA = 1000; // Hyper M12 Gravity: $10.0
...
void UpdateUI(double price, double ema, double vol, double vol_ma, double rsi_m3, double rsi_m12, double adx, bool is_uptrend, double b_trig, double s_trig, 
   ...
   DrawLabel("LQ_ST", 15, 350, "SYSTEM: " + (ords > 0 ? "MANAGING..." : (dst<=0 ? "READY!" : "HUNTING...")), 9, ords > 0 ? clrYellow : clrDeepSkyBlue);
   ChartRedraw(0);
}
