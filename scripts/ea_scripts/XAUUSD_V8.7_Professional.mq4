//+------------------------------------------------------------------+
//|                                   LiQiyu_Strategy_V8.7_Professional.mq4|
...
extern bool   Use_M1_Structure= true; // V8.7 M1 Pre-Squeeze & Candle Check
...
// --- Global Variables ---
int    Magic  = 2026807; // V8.7 Magic
string sComm  = "LQ_V8.7_Micro";
...
void OnTick() {
   ...
   // C. V8.7 Logic: M1 Micro-Structure
   // 1. M1 Pre-Breakout Squeeze (Quiet before storm) - Check Volume[1]
   bool m1_pre_sqz = !Use_M1_Structure || (iVolume(NULL, PERIOD_M1, 1) < vol_m1_avg * 1.3); // Relaxed trigger (1.3x Avg is still "normal" vs "spike")
   ...
   UpdateUI(M12_0.b_close, ema_m12, M3_1.b_volume, vol_ma_m3, rsi_m3, rsi_m12, adx_m30, ui_m30_sync, buy_trigger, sell_trigger, macd_buy_ok, macd_sell_ok, vol_ignite, ui_m3_sync, ui_mom_ok, m1_struct_ok);
}
...
//+------------------------------------------------------------------+
void UpdateUI(double price, double ema, double vol, double vol_ma, double rsi_m3, double rsi_m12, double adx, bool m30_ok, double b_trig, double s_trig, bool macd_buy_ok, bool macd_sell_ok, bool vol_ignite, bool m3_ok_trend, bool mom_ok, bool m1_struct_ok) {

   Comment("");   // No Comment Text
   color bg_color = clrBlack; 
   DrawRect("LQ_BG", 20, 20, 360, 480, bg_color); // Increased Height to 480

   // --- Header ---
   DrawLabel("LQ_Title", 40, 40, "LiQiyu V8.7 PRO (Micro-Structure)", 11, clrGold);
   ...
   string m1_s_txt = "    M1 Pre-Squeeze: " + (m1_struct_ok ? "QUIET (OK)" : "NOISY (Wait)");
   DrawLabel("LQ_M1S", 40, 210, m1_s_txt, 9, m1_struct_ok ? clrSilver : clrOrange); // Spaced 210

   string vi_txt = "    M1 Ignition: " + (vol_ignite ? "IGNITED (GO)" : "Low Energy");
   DrawLabel("LQ_VolIgnite", 40, 230, vi_txt, 9, vol_ignite ? clrLime : clrRed); // Spaced 230
   ...
   DrawLabel("LQ_RSI", 40, 260, r_txt, 10, rsi_all_ok ? clrLime : clrRed); // Spaced 260
   
   string r_d1 = "    M3=" + DoubleToString(rsi_m3, 1) + " / M12=" + DoubleToString(rsi_m12, 1);
   DrawLabel("LQ_RSI1", 40, 280, r_d1, 8, clrSilver); // Spaced 280
   
   string macd_txt = "    MACD Trend: " + ((macd_buy_ok || macd_sell_ok) ? "M12 CONFIRMED" : "CONFLICT");
   DrawLabel("LQ_MACD", 40, 300, macd_txt, 9, (macd_buy_ok || macd_sell_ok) ? clrLime : clrRed); // Spaced 300

   // --- Section 5: Breakout Zones (Sniper) ---
   double dist = 0;
   if (bull) {
       dist = (b_trig - Ask)/Point;
       DrawLabel("LQ_Trig", 40, 330, "M1 Breakout > " + DoubleToString(b_trig, Digits), 11, clrDeepSkyBlue);
       DrawLabel("LQ_Dist", 40, 350, "Relative: " + DoubleToString(dist, 0) + (dist <= 0 ? " (IN ZONE)" : " pts"), 10, dist <= 0 ? clrLime : clrGray);
   } else {
       dist = (Bid - s_trig)/Point;
       DrawLabel("LQ_Trig", 40, 330, "M1 Breakout < " + DoubleToString(s_trig, Digits), 11, clrOrangeRed);
       DrawLabel("LQ_Dist", 40, 350, "Relative: " + DoubleToString(dist, 0) + (dist <= 0 ? " (IN ZONE)" : " pts"), 10, dist <= 0 ? clrLime : clrGray);
   }
   ...
   bool ready = sqz && rsi_all_ok && adx_pass && m30_ok && (macd_buy_ok || macd_sell_ok) && m3_ok_trend && mom_ok && m1_struct_ok;
   ...
   DrawLabel("LQ_ST", 40, 420, st, 11, st_clr);
