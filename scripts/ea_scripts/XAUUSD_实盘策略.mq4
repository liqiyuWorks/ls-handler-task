...
//|                                   LiQiyu_Strategy_V8.0_Pro.mq4   |

//|                    M3/M12 Synthetic Trend + Volume Squeeze       |
...
int    Magic  = 2026803; // V8.3 Magic

string sComm  = "LQ_V8_Stable";
...
void OnTick() {
   ...
   SyntheticBar M3_1 = GetSyntheticBar(3, 1);   // 上一根 M3

   

   // 2. 计算基础指标

   double ema_m12 = GetSyntheticEMA(12, Trend_MA_Period, 0); // M12 趋势线

   double vol_ma_m3 = GetSyntheticVolMA(3, Vol_MA_Period, 1); // M3 过去成交量均值

   

   // 3. RSI 合成与共振

   double rsi_m3 = GetSyntheticRSI(3, 14);   // M3 周期 RSI(14)

   double rsi_m12 = GetSyntheticRSI(12, 14); // M12 周期 RSI(14)



   // 4. 高级趋势过滤 (V8.3 新增)

   // 4.1 ADX 趋势强度 (M30 周期, 稳健)
   ...
   if (signal == 0) OpenTrade(0, atr);

   if (signal == 1) OpenTrade(1, atr);
